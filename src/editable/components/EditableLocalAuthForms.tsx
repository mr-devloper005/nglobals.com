...
const inputClass = 'h-12 rounded-2xl border border-[var(--editable-border)] bg-white px-4 text-base font-bold text-[#11131c] outline-none transition placeholder:text-[#687083] focus:border-[#c6a063] focus:bg-white'
const buttonClass = 'inline-flex h-12 items-center justify-center rounded-2xl bg-[var(--editable-page-text,#11131c)] px-6 text-sm font-black uppercase tracking-[0.18em] text-[var(--editable-page-bg,#f6f3ee)] shadow-sm transition hover:-translate-y-0.5 disabled:opacity-60'
...
export function EditableLocalLoginForm() {
  ...
  return (
    <form className="mt-6 grid gap-4" onSubmit={submit}>
      ...
      {message ? <p className={`rounded-2xl px-4 py-3 text-sm font-bold ${status === 'success' ? 'bg-[#f6ecd9] text-[#11131c]' : 'bg-[#f6f3ee] text-[#11131c]'}`}>{message}</p> : null}
