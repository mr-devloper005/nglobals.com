...
function SearchResultCard({ post }: { post: SitePost }) {
  ...
  const taskLabel = SITE_CONFIG.tasks.find((item) => item.key === task)?.label || 'Company'
  return (
    <Link href={href} className="group block overflow-hidden rounded-xl border border-[var(--editable-border)] bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-xl">
      ...
      <div className="p-5">
        ...
        <h2 className="mt-4 line-clamp-2 text-xl font-black leading-tight tracking-[-0.03em] text-[var(--editable-page-text,#211713)]">{post.title}</h2>
        {summary ? <p className="mt-3 line-clamp-2 text-sm font-semibold leading-6 text-[var(--editable-page-text,#211713)]/65">{summary}</p> : null}
...
export default async function SearchPage({ searchParams }: { searchParams?: Promise<{ q?: string; category?: string; task?: string; master?: string }> }) {
  ...
  const task = (resolved.task || 'listing').trim().toLowerCase()
        ...
        <section className="mx-auto max-w-[1040px] px-4 py-8 sm:px-6 lg:px-8 lg:py-12">
          <div className="grid gap-7 rounded-xl border border-[var(--editable-border)] bg-white p-6 shadow-[0_18px_50px_rgba(0,0,0,0.07)] md:grid-cols-[0.82fr_1.18fr] lg:p-8">
            <div>
              ...
              <p className="mt-4 max-w-md text-sm font-semibold leading-7 opacity-70">{pagesContent.search.hero.description}</p>
            </div>
            <form action="/search" className="self-center rounded-xl border border-[var(--editable-border)] bg-[var(--editable-page-bg,#fff7ee)] p-4">
              ...
              <div className="mt-3 grid gap-3 sm:grid-cols-2">
                ...
                <select name="task" defaultValue={task} className="rounded-2xl border border-[var(--editable-border)] bg-white px-4 py-3 text-sm font-black outline-none">
                  <option value="listing">Business listings</option>
                  {enabledTasks.map((item) => <option key={item.key} value={item.key}>{item.label}</option>)}
                </select>
              </div>
              <button className="mt-3 inline-flex h-11 w-full items-center justify-center rounded-xl bg-[var(--editable-page-text,#2f1d16)] px-6 text-sm font-black uppercase tracking-[0.18em] text-[var(--editable-page-bg,#fff7ee)] transition hover:-translate-y-0.5" type="submit">Search</button>
            </form>
          </div>
          ...
          <div className="mt-10 flex flex-wrap items-end justify-between gap-4">
            ...
            <Link href="/listing" className="inline-flex items-center gap-2 rounded-full border border-[var(--editable-border)] bg-white px-5 py-3 text-sm font-black">Browse business listings <ArrowRight className="h-4 w-4" /></Link>
          </div>
          ...
          {results.length ? (
            ...
            <div className="mt-8 rounded-[2rem] border border-dashed border-[var(--editable-border)] bg-white/70 p-10 text-center">
              <p className="text-2xl font-black tracking-[-0.04em]">No matching companies found.</p>
              <p className="mt-3 text-sm font-semibold opacity-60">Try a different service, location, company name, or category.</p>
            </div>
          )}
        </section>
      </main>
