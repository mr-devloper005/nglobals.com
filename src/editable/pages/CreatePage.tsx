...
import { ArrowRight, CheckCircle2, Globe2, Lock, MapPin, Send, ShieldCheck } from 'lucide-react'
          ...
          <section className="mx-auto grid max-w-5xl gap-8 rounded-xl border border-[var(--editable-border)] bg-white p-7 shadow-[0_30px_90px_rgba(15,23,42,0.08)] md:grid-cols-[0.9fr_1.1fr] md:p-10">
            ...
            <div className="self-center">
              ...
              <div className="mt-8 flex flex-wrap gap-3">
                <Link href="/login" className="inline-flex items-center gap-2 rounded-lg bg-[#171922] px-6 py-3 text-sm font-black text-white">Login <ArrowRight className="h-4 w-4" /></Link>
                <Link href="/signup" className="inline-flex items-center gap-2 rounded-lg border border-[var(--editable-border)] bg-white px-6 py-3 text-sm font-black">Create account</Link>
              </div>
            </div>
          </section>
        </main>
        ...
        <section className="mx-auto max-w-[var(--editable-container)] px-4 py-10 sm:px-6 lg:px-8 lg:py-16">
          <div className="grid gap-8 rounded-xl border border-[var(--editable-border)] bg-white p-6 shadow-[0_30px_90px_rgba(15,23,42,0.08)] lg:grid-cols-[0.85fr_1.15fr] lg:p-10">
            <aside>
              ...
              <p className="mt-6 max-w-xl text-base font-semibold leading-8 text-[#4f5565]">{pagesContent.create.hero.description}</p>
              <div className="mt-8 grid gap-3">
                {[
                  [ShieldCheck, 'Verification-ready', 'Add clear details so buyers can trust and compare your company.'],
            ...
            <form onSubmit={submit} className="rounded-xl border border-[var(--editable-border)] bg-[#f6f3ee] p-5 sm:p-7">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  <p className="text-xs font-black uppercase tracking-[0.22em] text-[#c6a063]">Create {activeTask?.label || 'business listing'}</p>
              ...
              <div className="mt-6 grid gap-4">
                <input className={fieldClass} value={title} onChange={(event) => setTitle(event.target.value)} placeholder="Company name" required />
                <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  <input className={fieldClass} value={url} onChange={(event) => setUrl(event.target.value)} placeholder="Company website URL" />
                </div>
                <input className={fieldClass} value={image} onChange={(event) => setImage(event.target.value)} placeholder="Logo or showcase image URL" />
                <textarea className={`${fieldClass} min-h-24`} value={summary} onChange={(event) => setSummary(event.target.value)} placeholder="Short company summary" required />
                <textarea className={`${fieldClass} min-h-48`} value={body} onChange={(event) => setBody(event.target.value)} placeholder="Services, industries, pricing notes, team size, location, proof points, and contact details" required />
              </div>
              ...
              <button type="submit" className="mt-5 inline-flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-[#171922] px-6 text-sm font-black uppercase tracking-[0.18em] text-white transition hover:-translate-y-0.5">
